...
public class AsynchronousFileChannelTest1 {
	...
	public static void main(String[] args) {
		...
		try {
			channel = AsynchronousFileChannel.open(file, StandardOpenOption.CREATE, StandardOpenOption.READ,
					StandardOpenOption.WRITE);
			/*
			 * Becuase of the operations are synchronous. so the read maybe
			 * ended before write. You should test read/write just once
			 * everytime. 因为读写操作都是异步的，所以读操作也有可能在写操作前完成。 你应该每一次只测试其他一个IO操作，读或者写。
			 */
			//write(channel);
			read(channel);
